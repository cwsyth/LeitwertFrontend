# generate_geohash_geojson_country_aggregates.py
# Creates ONE Point per country_code, with router_count aggregated from the original 20k-router generator.
# Geometry is set to the center of the geohash cell of the aggregated (mean) location.
...
def main():
    random.seed(42)  # keep same seed to match your “original script” distribution

    N = 20000
    precision = 6
    eu_share = 0.55

    # Aggregate stats per country based on the same generation logic as the original script.
    # We aggregate using the geohash-cell centers for consistency with your data.
    per_country = {}  # cc -> {count, sum_lat, sum_lon}

    for _ in range(N):
        region = "EU" if random.random() < eu_share else "US"
        _city, clat, clon, w, _region, country_code = pick_city(region)

        lat, lon = sample_point_around(clat, clon, region, w)
        gh = geohash_encode(lat, lon, precision)
        latc, lonc = geohash_center(gh)

        s = per_country.get(country_code)
        if s is None:
            per_country[country_code] = {"count": 1, "sum_lat": latc, "sum_lon": lonc}
        else:
            s["count"] += 1
            s["sum_lat"] += latc
            s["sum_lon"] += lonc

    # Build ONE feature per country
    features = []
    for country_code, s in sorted(per_country.items()):
        count = s["count"]
        mean_lat = s["sum_lat"] / count
        mean_lon = s["sum_lon"] / count

        # Create a geohash for the aggregated “middle” and snap geometry to the center of that cell
        agg_gh = geohash_encode(mean_lat, mean_lon, precision)
        agg_latc, agg_lonc = geohash_center(agg_gh)

        features.append({
            "type": "Feature",
            "geometry": {"type": "Point", "coordinates": [agg_lonc, agg_latc]},
            "properties": {
                "type": "router_collection",
    ...
    out = "router_collections_by_country_p6.geojson"
    with open(out, "w", encoding="utf-8") as f:
        json.dump(fc, f, ensure_ascii=False)

    print(f"Wrote {out} with {len(features)} features (1 per country). Total routers aggregated: {sum(s['count'] for s in per_country.values())}")
