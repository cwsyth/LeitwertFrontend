...
export default function DashboardHeader({ viewVisibility, toggleView, selectedCountry, setSelectedCountry }: DashboardHeaderProps) {
    return (
        <div className="dashboard-header w-full rounded-[var(--radius)] overflow-hidden flex gap-3 items-stretch">
            <StatusCard title={"Autonome Systeme"} apiEndpoint={"http://localhost:5001/api/v1/router/health"} selectedCountry={selectedCountry} />
            <StatusCard title={"Router"} apiEndpoint={"http://localhost:5001/api/v1/router/health"} selectedCountry={selectedCountry} />
